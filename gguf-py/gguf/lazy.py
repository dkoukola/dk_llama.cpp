...
class LazyChunkedTensor:
    """A row-chunked tensor that never materializes all chunks together."""
    ...
    def squeeze(self) -> LazyChunkedTensor:
        shape = tuple(dim for dim in self.shape if dim != 1)
        if shape != self.shape:
            raise ValueError("squeezing a chunked tensor would change its row layout")
        return self
    ...
    def quantize(self, qtype: Any) -> LazyChunkedTensor:
        ...
        if qtype == GGMLQuantizationType.F32:
            ...
            try:
                ...
                raise QuantError(str(exc)) from exc
    ...
    def tofile(self, *args, **kwargs) -> None:
        ...
        for load_chunk in self._chunks:
            ...
            del chunk
        if written != self.nbytes:
            raise ValueError(f"chunked tensor wrote {written} bytes, expected {self.nbytes}")
